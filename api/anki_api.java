...
public class Client {
    ...
    public CompletableFuture<String[]> disconnect(boolean force_stop) throws IOException, InterruptedException {
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(base_url + "/disconnect?token=" + token))
                .header("Content-Type", "application/json")
                .POST(HttpRequest.BodyPublishers.ofString("{\"force_stop\":" + force_stop + "}"))
                .build();
