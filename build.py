...
class build_file():
    ...
    def compile(self, token: str):
        # Konfiguriere PyInstaller-Optionen
        filename = f"{token}.py"
        # Rufe PyInstaller auf, um die Python-Datei zu kompilieren
...
class build_project():
    def __init__(self, PROJECT, PATH, FILES) -> None:
        self.PROJECT, self.PATH, self.FILES = PROJECT, PATH, FILES
        self.options: list = OPTIONS
        
        with open("anki.json", "r") as file:
            self.compiler = json.load(file)
            
        with open(f"{FILES}/{self.PROJECT}.ap", "r") as file:
            self.fileList = file.read()
            self.text = self.readCode(self.text)
            
        self.token = secrets.token_hex(8)
        with open(f"{self.token}.py", "w", encoding="utf-8") as file:
            file.write(self.text)
