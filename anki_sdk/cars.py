...
class CarClass():
    ...
    def notifycallback(self, event: tuple[int, int] | Receive | Receive.Track | Receive.Connection = (17, 0)):
        """@decorator
    ...
    def ping(self):
        ...
        Returns:
            ping (int): Ping in ms.
        """
        value: int = asyncio.run(self.async_ping())
        return value
    ...
    def disconnect(self, stop_car: bool = True):
        ...
        """
        try:
            value: bool = asyncio.run(self.async_disconnect(stop_car=stop_car))
        except KeyboardInterrupt:
            value = True
        return value

    # disconnect and slow down the car
    async def async_disconnect(self, stop_car:bool = True):
        """Disconnect from vehicle
        ...
        async def loop_def():
            while True:
                if self.connected:
                    ...
                    await asyncio.sleep(1)  # Run for an hour
                else:
                    break
